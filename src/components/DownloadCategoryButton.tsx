...
export function DownloadCategoryButton({ products, categoryName }: DownloadCategoryButtonProps) {
  ...
  const handleDownload = async () => {
    ...
    // Usar Promise e setTimeout para permitir que a UI atualize
  ...
  // Função separada para gerar o PDF
  const generatePdf = () => {
    try {
  ...
  return (
    <Button
      ...
      className={`w-full ${isGenerating ? 'opacity-70' : 'bg-[#252525] hover:bg-[#333333]'} text-white`}
    >
      {isGenerating ? (
        <>
