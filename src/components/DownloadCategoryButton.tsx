...
export function DownloadCategoryButton({ products, categoryName }: DownloadCategoryButtonProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      doc.text('CATÁLOGO DIGITAL', doc.internal.pageSize.width / 2, Number(40), { align: 'center' });
      
      // Subtítulo (categoria)
      doc.setTextColor('#000000');
      doc.setFontSize(24);
      doc.text(categoryName.toUpperCase(), doc.internal.pageSize.width / 2, Number(60), { align: 'center' });
      ...
      for (const product of products) {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text(product.name, xPosition + 5, Number(yPosition + 65), {
        ...
        doc.setTextColor(colors.text);
        doc.setFontSize(8);
        doc.text(`COD: ${product.code}`, xPosition + 8, Number(yPosition + 80));
      ...
      doc.text('FAÇA SEU PEDIDO NOS MEIOS DE CONTATO ABAIXO:', 
        doc.internal.pageSize.width / 2, Number(40), { align: 'center' });

      // Informações de contato
      doc.setFontSize(14);
      doc.text('R. JACOB GREMMELMAIER, 409 - CENTRO', 
        doc.internal.pageSize.width / 2, Number(70), { align: 'center' });
      
      doc.setTextColor(colors.primary);
      doc.text('54 9927-0560', doc.internal.pageSize.width / 2 - 50, Number(90), { align: 'center' });
      doc.text('OU', doc.internal.pageSize.width / 2, Number(90), { align: 'center' });
      doc.text('54 9998-6916', doc.internal.pageSize.width / 2 + 50, Number(90), { align: 'center' });
